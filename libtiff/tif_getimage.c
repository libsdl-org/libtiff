        ...
        case PHOTOMETRIC_YCBCR:
            if ((img->bitspersample == 8) && (img->samplesperpixel == 3))
            {
                if (initYCbCrConversion(img) != 0)
                {
                    ...
                    /* Validate that the image dimensions are compatible with the subsampling block.
                    All putcontig8bitYCbCrXYtile routines assume width >= X and height >= Y. */
                    if (img->width < SubsamplingHor || img->height < SubsamplingVer)
                    {
                        TIFFErrorExt(img->tif, TIFFFileName(img->tif),
