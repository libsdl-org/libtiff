 ...
 * order to avoid having to transverse the entire directory list each time a new one is added. The offset is not stored
 ...
 * TIFFWriteDirectory(), which means the next directory write will transverse the entire list.
 *
...
int write_directory_to_open_file(TIFF *tif) {
...
int write_directory_to_closed_file(const char *filename, bool is_big_tiff) {
    ...
    if (write_directory_to_open_file(tif)) {
        fprintf(stderr, "Can't add new directory.\n");
        TIFFClose(tif);
        return 1;
    }
    TIFFClose(tif);
...
int main() {
    if (test_lastdir_offset(false)) {
        fprintf(stderr, "Failed during non-BigTIFF test.\n");
        return 1;
    }

    if (test_lastdir_offset(true)) {
        fprintf(stderr, "Failed during BigTIFF test.\n");
        return 1;
    }
    return 0;
}
